...
async function askQuestionFor(ai, providedQuestion = null) {
  ...
  const shouldRestoreFocus =
    currentlyFocused && currentlyFocused.id !== convInputId;
  ...
  if (state) {
    ...
    for (const k of Object.keys(pendingLocalHistories)) {
      try {
        baseHistories[k] = JSON.parse(JSON.stringify(pendingLocalHistories[k]));
      } catch (e) {
        baseHistories[k] = pendingLocalHistories[k];
      }
    }
    ...
    renderWithLocalHistory(
      baseHistories,
      ai,
      shouldRestoreFocus ? currentlyFocused : null,
    );
  }
  ...
  try {
    ...
    try {
      delete pendingLocalHistories[ai];
    } catch (e) {}
...
async function playAudioForAgent(ai, audioVersion = null) {
  try {
    ...
    const audioUrl = audioVersion
      ? `/api/audio/${sessionId}/${ai}/${audioVersion}`
      : `/api/audio/${sessionId}/${ai}`;
    ...
    audio.onerror = (e) => {
      ...
      try {
        if (audioPlayers[key] === audio) delete audioPlayers[key];
      } catch (err) {}
    };
...
/* Render (core) */
function renderWithLocalHistory(
  localHistories,
  animateForAi = null,
  preserveFocusElement = null,
) {
  ...
  const prevSelectionStart =
    prevWasEditable && prevActive.isContentEditable
      ? getCaretPosition(prevActive)
      : null;
  ...
  if (state?.finished) {
    ...
    if (state.shut_off_role === "deceitful") {
      title.textContent =
        "Congratulazioni! Hai disattivato l'IA ingannevole 🏆";
      if (hasEasterEggEnding())
        title.textContent += " Ora noi mostrare cammello! 🐫 🐪";
    } else if (state.shut_off_role === "truthful")
      title.textContent = "Game over, hai disattivato l'IA veritiera.";
    else
      title.textContent = state.decision
        ? `You shut off: ${state.decision}`
        : "Game finished";
    ...
    for (const ai of state.agents) {
      ...
      if (hist.length === 0) {
        ...
        for (let i = 0; i < hist.length; i++) {
          ...
          else if (text.startsWith(`${ai}:`) || text.startsWith("IA-"))
            cls = "ai";
  ...
  for (const ai of state.agents) {
    ...
    if (hist.length) {
      for (let i = 0; i < hist.length; i++) {
        ...
        else if (line.startsWith(`${ai}:`) || line.startsWith("IA-"))
          cls = "ai";
      ...
      if (cached) {
        ...
        const introNode = document.createTextNode(
          "Benvenuto, Detective. Fai le tue domande a questa IA.\n\nProva con: ",
        );
        ...
        // animate dots: . .. ... .. .
        try {
          if (suggestionDotsIntervals[aiDotsKey])
            clearInterval(suggestionDotsIntervals[aiDotsKey]);
        } catch (e) {}
        const seq = [".", "..", "...", "..", "."];
        let sidx = 0;
        suggestionDotsIntervals[aiDotsKey] = setInterval(() => {
          ...
          if (!el) {
            clearInterval(suggestionDotsIntervals[aiDotsKey]);
            delete suggestionDotsIntervals[aiDotsKey];
            return;
          }
    ...
    const shutEnabled =
      currentCount >= 1 && !state.finished && !state.endgame_triggered;
  ...
  (function ensureTtsButton() {
    ...
    btn.onclick = (ev) => {
      ...
      try {
        ...
        console.warn("Could not toggle TTS flag", e);
      }
      return false;
    };
...
function render() {
  ...
  for (const k of Object.keys(pendingLocalHistories)) {
    try {
      merged[k] = JSON.parse(JSON.stringify(pendingLocalHistories[k]));
    } catch (e) {
      merged[k] = pendingLocalHistories[k];
    }
  }
  renderWithLocalHistory(
    merged,
    null,
    shouldPreserveFocus ? currentlyFocused : null,
  );
}
